...
\hypertarget{part-introduction-uxe0-r}{%
\part{Introduction à R}\label{part-introduction-uxe0-r}}

\hypertarget{objectifs-labo-1}{%
\chapter{Objectifs Labo 1}\label{objectifs-labo-1}}
...
\chapter{Premier pas avec R}\label{premier-pas-avec-r}}
...
\section{La console}\label{la-console}}

\hypertarget{linvite-de-commandes}{%
\subsection{L'invite de commandes}\label{linvite-de-commandes}}

Au premier lancement de RStudio, l'écran principal est découpé en trois grandes zones~:

\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_start.png}
...
\subsection{Précisions concernant la saisie des commandes}\label{pruxe9cisions-concernant-la-saisie-des-commandes}}
...
symboles \texttt{.} et \texttt{\_}. Ils ne peuvent pas commencer par un
chiffre. Attention, R fait la différence entre minuscules et majuscules
dans les noms d'objets, ce qui signifie que \texttt{x} et \texttt{X}
seront deux objets différents, tout comme \texttt{resultat} et
\texttt{Resultat}.
...
exemple, on préfèrera comme nom d'objet \texttt{taille\_conj1} à
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diplome <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"CAP"}\NormalTok{, }\StringTok{"Bac"}\NormalTok{, }\StringTok{"Bac+2"}\NormalTok{, }\StringTok{"CAP"}\NormalTok{, }\StringTok{"Bac+3"}\NormalTok{)}
\NormalTok{diplome}
\end{Highlighting}
\end{Shaded}
...
Ceci explique le \texttt{{[}1{]}} qu'on obtient quand on affiche un simple nombre\footnote{Et permet de constater que pour R, un nombre est un vecteur à un seul élément.}~:
...
\subsection{Aide sur une fonction}\label{aide-sur-une-fonction}}
...
\section{Regrouper ses commandes dans des scripts}\label{regrouper-ses-commandes-dans-des-scripts}}
...
\section{\texorpdfstring{Installer et charger des extensions (\emph{packages})}{Installer et charger des extensions (packages)}}\label{packages}}
...
\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_package_install.png}
\caption{Installer une extension}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_package_install2.png}
\caption{Installation d'une extension}
\end{figure}
...
\section{Exercices}\label{ex-introR}}
...
\chapter{Premier travail avec des données}\label{premier-travail-avec-des-donnuxe9es}}

\hypertarget{jeu-de-donnuxe9es-dexemple}{%
\section{Jeu de données d'exemple}\label{jeu-de-donnuxe9es-dexemple}}
...
Le jeu de données que nous allons utiliser est un extrait de l'enquête \emph{Histoire de vie} réalisée par l'INSEE en 2003. Il contient 2000 individus et 20 variables. Pour une description plus complète et une liste des variables, voir la section \ref{hdv2003}.
...
\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_environment.png}
\caption{Onglet \emph{Environment}}
\end{figure}

Cet objet est d'un type nouveau~: il s'agit d'un tableau de données.

\hypertarget{tableau-de-donnuxe9es-data-frame}{%
\section{\texorpdfstring{Tableau de données (\emph{data frame})}{Tableau de données (data frame)}}\label{tableau-de-donnuxe9es-data-frame}}
...
\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_view_icon.png}
\caption{View icon}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_view.png}
...
\begin{rmdnote}
Sous R, on peut importer ou créer autant de tableaux de données qu'on le
souhaite, dans les limites des capacités de sa machine.
\end{rmdnote}
...
\subsection{Structure du tableau}\label{structure-du-tableau}}
...
\begin{figure}
\centering
\includegraphics{images/screenshots/rstudio_environment_str.png}
\caption{Structure d'un objet}
\end{figure}

\hypertarget{accuxe9der-aux-variables-dun-tableau}{%
\subsection{Accéder aux variables d'un tableau}\label{accuxe9der-aux-variables-dun-tableau}}
...
\subsection{Créer une nouvelle variable}\label{cruxe9er-une-nouvelle-variable}}
...
\subsection{Analyser une variable quantitative}\label{analyser-une-variable-quantitative}}
...
\subsubsection{Indicateurs de centralité}\label{indicateurs-de-centralituxe9}}
...
\subsubsection{Indicateurs de dispersion}\label{indicateurs-de-dispersion}}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-94-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(d}\OperatorTok{$}\NormalTok{age, }\DataTypeTok{breaks =} \DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-95-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(d}\OperatorTok{$}\NormalTok{age, }\DataTypeTok{breaks =} \DecValTok{70}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-96-1.pdf}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-97-1.pdf}

La fonction \texttt{hist} fait partie des fonctions graphique de base de R. On verra plus en détail d'autres fonctions graphiques dans la partie \ref{ggplot2} de ce document, consacrée à l'extension \texttt{ggplot2}, qui fait partie du \emph{tidyverse} et qui permet la production et la personnalisation de graphiques complexes.

\hypertarget{analyser-une-variable-qualitative}{%
\subsection{Analyser une variable qualitative}\label{analyser-une-variable-qualitative}}
...
\subsubsection{Tri à plat}\label{tri-uxe0-plat}}
...
\begin{rmdimportant}
Attention, par défaut la fonction \texttt{table} n'affiche pas les
valeurs manquantes (\texttt{NA}). Si on souhaite les inclure il faut
utiliser l'argument \texttt{useNA\ =\ "always"}, soit~:
\texttt{table(d\$qualif,\ useNA\ =\ "always")}.
\end{rmdimportant}
...
\subsubsection{Représentations graphiques}\label{repruxe9sentations-graphiques}}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-108-1.pdf}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-109-1.pdf}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-110-1.pdf}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-111-1.pdf}

\hypertarget{ex-introR-univ}{%
\section{Exercices}\label{ex-introR-univ}}
...
\chapter{Analyse de 2 variables}\label{analyse-de-2-variables}}
...
\section{Croisement de deux variables qualitatives}\label{croisement-de-deux-variables-qualitatives}}

\hypertarget{tableaux-croisuxe9s}{%
\subsection{Tableaux croisés}\label{tableaux-croisuxe9s}}
...
\hypertarget{test-du-ux3c7uxb2}{%
\subsection{Test du χ²}\label{test-du-ux3c7uxb2}}
...
Pour effectuer un test de ce type, on applique la fonction \texttt{chisq.test} au tableau croisé calculé précédemment~:
...
\tightlist
\item
  \texttt{X-squared}, la valeur de la statistique du χ² pour notre tableau, c'est-à-dire une ``distance'' entre notre tableau observé et celui attendu si les deux variables étaient indépendantes.
\item
  \texttt{df}, le nombre de degrés de libertés du test.
\item
  \texttt{p-value}, le fameux \emph{p}, qui indique la probabilité d'obtenir une valeur de la statistique du χ² au moins aussi extrême sous l'hypothèse d'indépendance.
\end{itemize}

Ici, le \emph{p} est extrêmement petit (la notation \texttt{\textless{}\ 2.2e-16} indique qu'il est plus petit que la plus petite valeur proche de zéro calculable par R), donc certainement en-dessous du seuil de décision choisi préalablement au test (souvent 5\%, soit 0.05). On peut donc rejeter l'hypothèse d'indépendance des lignes et des colonnes du tableau.

En complément du test du χ², on peut aussi regarder les \emph{résidus} de ce test pour affiner la lecture du tableau. Ceux-ci s'obtiennent avec la fonction \texttt{chisq.residuals} de \texttt{questionr}~:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{chisq.residuals}\NormalTok{(tab)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-125-1.pdf}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-126-1.pdf}
...
\section{Croisement d'une variable quantitative et d'une variable qualitative}\label{croisement-dune-variable-quantitative-et-dune-variable-qualitative}}

\hypertarget{repruxe9sentation-graphique-1}{%
\subsection{Représentation graphique}\label{repruxe9sentation-graphique-1}}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{boxplot}\NormalTok{(d}\OperatorTok{$}\NormalTok{age }\OperatorTok{~}\StringTok{ }\NormalTok{d}\OperatorTok{$}\NormalTok{sport)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-129-1.pdf}
...
\subsection{Calculs d'indicateurs}\label{calculs-dindicateurs}}
...
On verra dans la partie \ref{dplyr} d'autres méthodes basées sur \texttt{dplyr} pour effectuer ce genre d'opérations.

\hypertarget{tests-statistiques}{%
\subsection{Tests statistiques}\label{tests-statistiques}}

Un des tests les plus connus est le test du \emph{t} de Student, qui permet de tester si les moyennes de deux sous-populations peuvent être considérées comme différentes (compte tenu des fluctuations aléatoires provenant du biais d'échantillonnage).

Un test \emph{t} s'effectue à l'aide de la fonction \texttt{t.test}. Ainsi, on peut tester l'hypothèse d'égalité des âges moyens selon la pratique ou non d'un sport avec la commande suivante~:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{t.test}\NormalTok{(d}\OperatorTok{$}\NormalTok{age }\OperatorTok{~}\StringTok{ }\NormalTok{d}\OperatorTok{$}\NormalTok{sport)}
\end{Highlighting}
\end{Shaded}
...
Le résultat du test est significatif, avec un \emph{p} extrêmement petit, et on peut rejeter l'hypothèse nulle d'égalité des moyennes des deux groupes. Le test nous donne même un intervalle de confiance à 95\% pour la valeur de la différence entre les deux moyennes.

Nous sommes cependant allés un peu vite, et avons négligé le fait que le test \emph{t} s'applique normalement à des distributions normales. On peut se faire un premier aperçu visuel de cette normalité en traçant les histogrammes des deux répartitions~:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{hist}\NormalTok{(d_sport}\OperatorTok{$}\NormalTok{age)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-138-1.pdf}
...
\section{Croisement de deux variables quantitatives}\label{croisement-de-deux-variables-quantitatives}}
...
\subsection{Représentation graphique}\label{repruxe9sentation-graphique-2}}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-143-1.pdf}
...
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-144-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(rp2012}\OperatorTok{$}\NormalTok{cadres, rp2012}\OperatorTok{$}\NormalTok{dipl_sup)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-145-1.pdf}

\hypertarget{calcul-dindicateurs}{%
\subsection{Calcul d'indicateurs}\label{calcul-dindicateurs}}
...
\hypertarget{corruxe9lation-linuxe9aire-pearson}{%
\subsubsection{Corrélation linéaire (Pearson)}\label{corruxe9lation-linuxe9aire-pearson}}

La corrélation est une mesure du lien d'association \emph{linéaire} entre deux variables quantitatives. Sa valeur varie entre -1 et 1. Si la corrélation vaut -1, il s'agit d'une association linéaire négative parfaite. Si elle vaut 1, il s'agit d'une association linéaire positive parfaite. Si elle vaut 0, il n'y a aucune association linéaire entre les variables.

On la calcule dans R à l'aide de la fonction \texttt{cor}.

Ainsi la corrélation entre le pourcentage de cadres et celui de diplômés du supérieur vaut~:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor}\NormalTok{(rp2012}\OperatorTok{$}\NormalTok{cadres, rp2012}\OperatorTok{$}\NormalTok{dipl_sup)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor}\NormalTok{(rp2012}\OperatorTok{$}\NormalTok{cadres, rp2012}\OperatorTok{$}\NormalTok{proprio)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{corruxe9lation-des-rangs-spearman}{%
\subsubsection{Corrélation des rangs (Spearman)}\label{corruxe9lation-des-rangs-spearman}}
...
\section{Exercices}\label{ex-introR-biv}}
...
\CommentTok{## On peut rejeter l'hypothèse d'indépendance au seuil de 0.05,}
\CommentTok{## et même au seuil de 0.001}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cor}\NormalTok{(rp2012}\OperatorTok{$}\NormalTok{dipl_aucun, rp2012}\OperatorTok{$}\NormalTok{proprio)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{part-analyse-de-puissance}{%
\part{Analyse de puissance}\label{part-analyse-de-puissance}}
...
\hypertarget{part-moduxe8le-univariuxe9}{%
\part{Modèle univarié}\label{part-moduxe8le-univariuxe9}}
...
## t1* 1.1919926  0.001818556  0.03414086
## t2* 0.3408557 -0.001284031  0.02684973
\end{verbatim}
...
## Intervals : 
## Level      Normal              Basic         
## 95%   ( 1.123,  1.257 )   ( 1.120,  1.255 )  
## 
## Level     Percentile            BCa          
## 95%   ( 1.129,  1.264 )   ( 1.118,  1.256 )  
...
## Intervals : 
## Level      Normal              Basic         
## 95%   ( 0.2895,  0.3948 )   ( 0.2902,  0.3966 )  
## 
## Level     Percentile            BCa          
## 95%   ( 0.2851,  0.3915 )   ( 0.2903,  0.3962 )  
## Calculations and Intervals on Original Scale
\end{verbatim}
...
## Bootstrap Statistics :
##      original      bias    std. error
## t1* 45.084391 -0.01020777   0.4416771
## t2* -1.714546  0.02331378   0.7747569
\end{verbatim}
...
## Linear Hypotheses:
##                    Estimate lwr      upr     
##  1958 -  1954 == 0   0.1872  -3.2695   3.6440
##  1965 -  1954 == 0  -5.5077  -9.9950  -1.0203
##  1966 -  1954 == 0  -3.3127  -6.3417  -0.2837
##  1965 -  1958 == 0  -5.6949 -10.4090  -0.9808
##  1966 -  1958 == 0  -3.4999  -6.8556  -0.1442
##  1966 -  1965 == 0   2.1950  -2.2151   6.6050
\end{verbatim}
...
\begin{figure}
\centering
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-171-1.pdf}
\caption{\label{fig:unnamed-chunk-171-1}Exemple de graphique tiré du fichier d'aide de la fonction stat\_bin}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-171-2.pdf}
\caption{\label{fig:unnamed-chunk-171-2}Exemple de graphique tiré du fichier d'aide de la fonction stat\_bin}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-171-3.pdf}
\caption{\label{fig:unnamed-chunk-171-3}Exemple de graphique tiré du fichier d'aide de la fonction stat\_bin}
\end{figure}
