...
```{r error-on, echo = FALSE}
knitr::opts_chunk$set(error = TRUE)
```

```{r, fig.cap = "Conditions d'applications du modèle anova.model4"}
par(mfrow = c(2,2))
plot(anova.model4)
```

```{r error-off, echo = FALSE}
...
```{r, fig.cap = "Conditions d'applications du modèle anova.nested", echo = FALSE}
anova(anova.nested)
```
...
```{r, fig.cap = "Conditions d'applications du modèle anova.simple", echo = FALSE}
par(mfrow = c(2, 2))
plot(anova.simple)
```
