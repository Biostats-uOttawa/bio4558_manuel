...
## t1* 1.1919926  2.591634e-04  0.03414289
## t2* 0.3408557 -8.174073e-06  0.02704263
\end{verbatim}
...
## Intervals : 
## Level      Normal              Basic         
## 95%   ( 1.125,  1.259 )   ( 1.123,  1.259 )  
## 
## Level     Percentile            BCa          
## 95%   ( 1.125,  1.261 )   ( 1.121,  1.254 )  
...
## Intervals : 
## Level      Normal              Basic         
## 95%   ( 0.2879,  0.3939 )   ( 0.2881,  0.3951 )  
## 
## Level     Percentile            BCa          
## 95%   ( 0.2866,  0.3936 )   ( 0.2899,  0.3966 )  
## Calculations and Intervals on Original Scale
\end{verbatim}
...
## Bootstrap Statistics :
##      original       bias    std. error
## t1* 45.084391  0.003011076   0.4323156
## t2* -1.714546 -0.003823043   0.7849926
\end{verbatim}
...
require(car)
scatterplotMatrix(\textasciitilde{}logherp + logarea + cpfor2 + thtden +
swamp,
...
model.loga\textless{}-lm(logherp \textasciitilde{} logarea, data = mydata)
summary(model.loga)

opar \textless{}- par(mfrow(2,2))
plot(model.loga)
opar
...
 Refaites la régression de logherp sur logarea et cpfor2 à la fois, soit que logherp\textasciitilde{}logarea+cpfor2 . Que concluez-vous?
...
 Refaites la régression d'en haut, mais cette fois incluez un terme pour représenter ( swamp ) 2 . L'expression devrait apparaître comme: residuals\textasciitilde{}SWAMP+SWAMP\^{}2 . Que concluez-vous? Qu'est-ce que l'examen des résidus de cette régression multiple révèle?
...
\KeywordTok{require}\NormalTok{(simpleboot)}
\CommentTok{# R is the number of bootstrap iterations}
\CommentTok{# Setting rows to FALSE indicates resampling of residuals}
\NormalTok{mysimpleboot<-}\KeywordTok{lm.boot}\NormalTok{(mymodel, }\DataTypeTok{R =}\NormalTok{ nboot, }\DataTypeTok{rows =} \OtherTok{FALSE}\NormalTok{)}
\CommentTok{# Extract bootstrap coefficients}
\NormalTok{myresults<-}\KeywordTok{sapply}\NormalTok{(mysimpleboot}\OperatorTok{$}\NormalTok{boot.list, }\ControlFlowTok{function}\NormalTok{ (x)}
\NormalTok{x}\OperatorTok{$}\NormalTok{coef)}
\CommentTok{# Transpose matrix so that lines are bootstrap iterations}
\NormalTok{and columns are coefficients}
\NormalTok{tmyresults<-}\KeywordTok{t}\NormalTok{(myresults)}
\CommentTok{#}
\NormalTok{Plot histograms of bootstrapped coefficients}
\NormalTok{ncoefs<-}\KeywordTok{length}\NormalTok{(}\KeywordTok{data.frame}\NormalTok{(tmyresults))}
\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow=}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{1}\NormalTok{), }\DataTypeTok{mai=}\KeywordTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\FloatTok{0.5}\NormalTok{), }\DataTypeTok{ask =} \OtherTok{TRUE}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1} \OperatorTok{:}\StringTok{ }\NormalTok{ncoefs) \{}
\NormalTok{lab<-}\KeywordTok{colnames}\NormalTok{(tmyresults)[i]}
\NormalTok{x<-tmyresults[,i]}
...
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{v=}\NormalTok{mymodel}\OperatorTok{$}\NormalTok{coef[i], }\DataTypeTok{col =} \StringTok{"red"}\NormalTok{)}
\NormalTok{\}}
\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow=}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{), }\DataTypeTok{ask =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{\}}
\CommentTok{# bootstrapping with 1000 replications}
\NormalTok{results <-}\StringTok{ }\KeywordTok{boot}\NormalTok{(}\DataTypeTok{data=}\NormalTok{mydata, }\DataTypeTok{statistic=}\NormalTok{bs,}
\DataTypeTok{R=}\DecValTok{1000}\NormalTok{, }\DataTypeTok{formula=}\NormalTok{logherp }\OperatorTok{~}\StringTok{ }\NormalTok{logarea }\OperatorTok{+}\StringTok{ }\NormalTok{thtden }\OperatorTok{+}\StringTok{ }\NormalTok{swamp }\OperatorTok{+}
\KeywordTok{I}\NormalTok{(swamp}\OperatorTok{^}\DecValTok{2}\NormalTok{))}
\CommentTok{# view results}
\NormalTok{results}
\KeywordTok{plot}\NormalTok{(results, }\DataTypeTok{index=}\DecValTok{1}\NormalTok{) }\CommentTok{# intercept}
...
\caption{\label{fig:unnamed-chunk-252-1}Exemple de graphique tiré du fichier d'aide de la fonction stat\_bin}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-252-2.pdf}
\caption{\label{fig:unnamed-chunk-252-2}Exemple de graphique tiré du fichier d'aide de la fonction stat\_bin}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{Labo_BIO4558_files/figure-latex/unnamed-chunk-252-3.pdf}
\caption{\label{fig:unnamed-chunk-252-3}Exemple de graphique tiré du fichier d'aide de la fonction stat\_bin}
\end{figure}
